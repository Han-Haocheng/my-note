...
> ![202410071006.svg](数学/attachments/202410071006.svg)
> - 区域分别是 $D_{1},D_{2},S$
...
> [!faq] 例题 1：计算 $\iint_{D}xy \mathrm{d}\sigma$ ,其中 D 是直线 y=1 、x=2 及 y=x 所围成的闭区域.
> 
> 解
> 
> ![202410071016.svg](数学/attachments/202410071016.svg)
> - 画出积分区域 D
> - 使用 x 型 $$\int_{1}^{2} \, dx\int_{1}^{x} xy \, dy=\frac{9}{8}$$
> - 使用 y 型 $$\int_{1}^{2}\, dy\int_{y}^{2}xy\, dx=\frac{9}{8}$$

> [!faq]  例题 2：计算 $\iint_{D}y\sqrt{ 1+x^{2}-y^{2} }\mathrm{d}\sigma$，其中 D 是由直线 $y=x$、$x=-1$ 和 $y=1$ 所围成的封闭区域
解
> - 画出积分区域 D
> ![202410071017.svg](数学/attachments/202410071017.svg)
> - 使用 x 型 $$\int_{-1}^{1}\, dx  \int_{x}^{1} y\sqrt{ 1+x^{2}-y^{2} }\, dy=\frac{1}{2}$$
> - 使用 y 型 $$\int_{-1}^{1}\, dy  \int_{-1}^{y} y\sqrt{1+x^{2}-y^{2}}\, dx = 麻烦$$

> [!faq] 例题 3：计算 $\iint_{D}xy\mathrm{d}\sigma$，其中 D 是由抛物线 $y^{2}=x$ 及直线 $y=x-2$ 所围成的闭区域
> 解
> - 画积分区域 D
> ![202410071018.svg](数学/attachments/202410071018.svg)
...
> [!faq] 例题 4：求两个底圆半径都等于 R 的直交圆柱面所围成的立体体积
> 
> 解
> - 设这两个圆柱面的方程为
> 	- $x^{2}+y^{2}=R^{2}$
> 	- $x^{2}+z^{2}=R^{2}$
> - 画积分区域
> ![202410071019.svg](数学/attachments/202410071019.svg)
...
> [!faq] 例题 5：计算 $\iint_{D}e^{-x^{2}-y^{2}}\mathrm{d}x\mathrm{d}y$ ,其中 D 是圆心在原点，半径为 $\alpha$ 的圆周所围成的闭区域。
> 
> 解
> 
> - 闭区域 D
...
> [!faq]  例题 6：求球体 $x^{2}+y^{2}+z^{2}\leq 4a^{2}$ 被圆柱面 $x^{2}+y^{2}=2ax(a>0)$ 所截得的（含在圆柱面内的部分）立体体积
> 
> 解
> 
...
> - 画积分区域
> 
> ![[数学/attachments/20250104152553.svg]]
...
> [!faq]  例题 7：计算 $\iint_{d}e^{\frac{y-x}{y+x}}\mathrm{d}x\mathrm{d}y$，其中 $D$ 是由 $x$ 轴、$y$ 轴和直线 $x+y=2$ 所围成的闭区域
> 
> 解
> 
> - 画区域图
