...
#数学/公式/积分 $\Gamma$ 函数：$$\Gamma(s)=\int _{0}^{\infty}e^{-x}x^{s-1} \, dx $$


 #数学/公式/积分 $\Gamma$函数递推公式$$\Gamma(s+1)=s\Gamma(s)=s!(s>0)$$
