...
<font color="#f79646">读取用户输入的字符串，并把这些字符串写入到input.txt文件中，直到读取到空字符串为止</font>
...
13.  设有数组定义：`int a[] = {11,22,33,44,55,66,77,88,99};`  
则执行以下语句后的输出结果是（      ） .  （      ） .  （      ） 。
```java
...
public class R {
    public static void main(String[] args) {
        double l = Double.parseDouble(JOptionPane.showInputDialog("长："));
        double w = Double.parseDouble(JOptionPane.showInputDialog("宽："));
        double p = 2 * (l + w);
        double a = l * w;
        JOptionPane.showMessageDialog(null, "周长：" + p + "\n面积：" + a);
        System.exit(0);
    }
}
	```
2. 用Java Application编写一个程序，输出你的班级和姓名。
	...
	public class output{
		public static void main(String args[]){
			System.out.println("班级：二班");
			System.out.println("姓名：小耿");
		}
	}
	```
3. 按以下要求编写程序
