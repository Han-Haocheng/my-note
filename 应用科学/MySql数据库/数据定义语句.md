...
NULL 约束：即要么是 NULL 要么是 NOT NULL
- `NULL` :默认，可允许在插入记录时不给出该列的值
- `NOT NULL` :表的记录该列必须含有值

自增属性 ``AUTO_INCREMENT``：为整型列设置的属性。需定义*主键约束*或*候选键约束*
- 默认的初始值为1

默认值 `DEFAULT`：在插入数据时，没有明确给出某个字段所对应的值，则 DBMS 将为该字段指定这个默认值
